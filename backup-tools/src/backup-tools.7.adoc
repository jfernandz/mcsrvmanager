...
Optional top-level `specific_targets:` defines a separate generic backup job.
`specific_targets:` may be either a list of paths or a mapping with `dirs` and optional overrides (including `owner`).
...
If top-level `specific_targets:` is present and non-empty:

- `backup@specific_targets.timer` (targets `backup@specific_targets.service`)
- `<service-config-dir>/specific_targets.conf`
